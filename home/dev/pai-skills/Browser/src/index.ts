...
// Detect Chrome executable path for NixOS and other systems
function detectChromePath(): string | undefined {
  const paths = [
    '/run/current-system/sw/bin/google-chrome-stable',  // NixOS
  ...
  for (const p of paths) {
    try {
      // Use Bun's file API for sync check
      if (Bun.file(p).size !== undefined) return p
    } catch {
...
export class PlaywrightBrowser {
  ...
  async launch(options?: LaunchOptions): Promise<void> {
    ...
    // Use provided path, or detect system Chrome for NixOS compatibility
    const executablePath = options?.executablePath ||
      (browserType === 'chromium' ? detectChromePath() : undefined)
